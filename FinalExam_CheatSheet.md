...
energy = -232.123456
# 基本用法
print(f"Molecule: {name}, Energy: {energy:.2f} Ha")
# 输出: Molecule: Benzene, Energy: -232.12 Ha
```
...
| **`w`**       | 设定最小总宽度    | `f"{12:5d}"`        | `' 12'` (前面补空格) |
| **`0w`**      | 宽度不足补零     | `f"{12:05d}"`       | `'00012'`       |
| **`,`**       | 千分位分隔符     | `f"{1000000:,}"`    | `'1,000,000'`   |
| **`e` / `E`** | **科学计数法**  | `f"{0.00001: .2e}"` | `'1.00e-05'`    |
| **`%`**       | 百分比格式      | `f"{0.85: .1%}"`    | `'85.0%'`       |
#### 对齐与填充

这在打印整齐的 **原子坐标表** 或 **矩阵** 时极其重要：
...
```python
# 示例：打印对齐的表头
header = f"{'Atom':<10}{'X':^10}{'Y':^10}{'Z':^10}"
print(header)
# Atom          X         Y         Z     
